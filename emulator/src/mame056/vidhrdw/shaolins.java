	...
	***************************************************************************/
	...
	public static VhConvertColorPromPtr shaolins_vh_convert_color_prom = new VhConvertColorPromPtr() {
		...
		for (i = 0;i < Machine.drv.total_colors;i++)
		{
			...
			palette[_posPal++] = (char) (0x0e * bit0 + 0x1f * bit1 + 0x43 * bit2 + 0x8f * bit3);
	...
	public static VhUpdatePtr shaolins_vh_screenrefresh = new VhUpdatePtr() { public void handler(mame_bitmap bitmap,int full_refresh) 
	{
		...
		/* copy the temporary bitmap to the screen */
		{
			...
			for (i = 0;i < 4;i++)
				scroll[i] = 0;
			for (i = 4;i < 32;i++)
				scroll[i] = -shaolins_scroll.read(0)-1;
		...
		for (offs = spriteram_size[0]-32; offs >= 0; offs-=32 ) /* max 24 sprites */
		{
			if (((spriteram.read(offs)!=0) && (spriteram.read(offs+6)!=0))) /* stop rogue sprites on high score screen */
			{
				drawgfx(bitmap,Machine.gfx[1],
						...
						((spriteram.read(offs+9) & 0x40)==0)?1:0,(spriteram.read(offs+9) & 0x80),
