...
public class galaga
{
	...
	static int MAX_STARS = 250;
	static int STARS_COLOR_BASE = 32;
	...
	***************************************************************************/
	...
	public static VhConvertColorPromPtr galaga_vh_convert_color_prom = new VhConvertColorPromPtr() {
		...
		for (i = 0;i < TOTAL_COLORS(0);i++){
			COLOR(colortable,0,i,15 - ((color_prom.read()) & 0x0f));
                        color_prom.inc();
                }
		...
		{
			if (i % 4 == 0) COLOR(colortable,1,i,0);	/* preserve transparency */
			else COLOR(colortable,1,i,15 - ((color_prom.read() & 0x0f)) + 0x10);
