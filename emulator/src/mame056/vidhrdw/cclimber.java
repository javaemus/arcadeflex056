...
public class cclimber
{
	...
	***************************************************************************/
	...
	public static VhConvertColorPromPtr cclimber_vh_convert_color_prom = new VhConvertColorPromPtr() {
		...
		for (i = 0;i < TOTAL_COLORS(0);i++)
		{
			...
			if (i % 4 == 0) COLOR(colortable,0,i,0);
			else COLOR(colortable,0,i,i);
		}
		...
		{
			if (i % 4 == 0) COLOR(colortable,2,i,0);
			else COLOR(colortable,2,i,i + 64);
		}
	}};
	...
	***************************************************************************/
	...
	public static VhConvertColorPromPtr swimmer_vh_convert_color_prom= new VhConvertColorPromPtr() {
		...
		for (i = 0;i < 256;i++)
		{
			...
			palette[_palPos++] = (char) (0x20 * bit0 + 0x40 * bit1 + 0x80 * bit2);
			/* green component */
			...
			if ((i % 8)!=0)
			{
			    ...
			    COLOR(colortable,0,i+256,i);
			}
			else
			{
				/* background */
				COLOR(colortable,0,i,BGPEN);
				COLOR(colortable,0,i+256,SIDEPEN);
			}
		}
		...
		for (i = 0;i < 32;i++)
		{
			...
			if (i % 8 == 0) COLOR(colortable,2,i,BGPEN);  /* enforce transparency */
			else COLOR(colortable,2,i, i+256);
		}
	...
	/*TODO*///	*(palette++) = 0x4e;
	/*TODO*///#endif
		palette[_palPos++] = 0x20;
		palette[_palPos++] = 0x98;
		palette[_palPos++] = 0x79;
	}};
	...
	***************************************************************************/
	static void drawbigsprite(mame_bitmap bitmap)
	{
		...
		color = cclimber_bigspriteram.read(1) & 0x07;	/* cclimber */
		...
		/* copy the temporary bitmap to the screen */
		{
			...
			if (flip_screen_x[0] != 0)
			{
				for (offs = 0;offs < 32;offs++)
				{
					...
					if (flip_screen_y[0] != 0) scroll[offs] = -scroll[offs];
				}
			}
			else
			{
