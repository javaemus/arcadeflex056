...
public class taitosj
{
	...
	static int[] dirtycharacter1=new int[256],dirtycharacter2=new int[256];
	static int[] dirtysprite1=new int[64],dirtysprite2=new int[64];
	...
	***************************************************************************/
	...
	public static ReadHandlerPtr taitosj_gfxrom_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		if (taitosj_gfxpointer.read(0) == 0) taitosj_gfxpointer.write(0, (taitosj_gfxpointer.read(1)+1));
	...
	***************************************************************************/
	...
	public static int get_sprite_xy(int num, int sx, int sy)
	{
		int offs = num * 4;
				...
				for (j = 0x00; j < 0x20; j++)
				{
					...
					if (get_sprite_xy(j, sx2, sy2) != 0)
					{
						...
						if ((abs(sx1 - sx2) < 16) &&
							(abs(sy1 - sy2) < 16))
						{
							if (check_sprite_sprite_bitpattern(sx1, sy1, i, sx2, sy2, j) != 0)
							{
