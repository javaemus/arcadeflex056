...
public class segar
{
	...
	static UBytePtr segar_characterram = new UBytePtr();
	static UBytePtr segar_characterram2 = new UBytePtr();
	static UBytePtr segar_mem_colortable = new UBytePtr();
	static UBytePtr segar_mem_bcolortable = new UBytePtr();
	
	public static class SEGAR_VID_STRUCT
	{
		public char[] dirtychar = new char[256];		// graphics defined in RAM, mark when changed
	
		public char[] colorRAM = new char[0x40];		// stored in a 93419 (vid board)
		public char[] bcolorRAM = new char[0x40];		// stored in a 93419 (background board)
		public int color_write_enable;	// write-enable the 93419 (vid board)
		public int flip;					// cocktail flip mode (vid board)
		public int bflip;				// cocktail flip mode (background board)
	
		public int refresh;				// refresh the screen
		public int brefresh;				// refresh the background
		public int char_refresh;			// refresh the character graphics
	
		public int has_bcolorRAM;		// do we have background color RAM?
		public int background_enable;	// draw the background?
		public int back_scene;
		public int back_charset;
	
		// used for Pig Newton
		public int bcolor_offset;
	
		// used for Space Odyssey
		public int backfill;
		public int fill_background;
		public int backshift;
		public mame_bitmap horizbackbitmap;
		public mame_bitmap vertbackbitmap;
	};
	...
	***************************************************************************/
	...
	public static VhConvertColorPromPtr segar_init_colors = new VhConvertColorPromPtr() {
            public void handler(char[] palette, char[] colortable, UBytePtr color_prom) {
                char color_scale[] = {0x00, 0x40, 0x80, 0xC0 };
		int i;
                int _palette = 0;
		...
		*/
		...
		{
			palette[_palette++] = color_scale[((i & 0x30) >> 4)];
			palette[_palette++] = color_scale[((i & 0x0C) >> 2)];
			palette[_palette++] = color_scale[((i & 0x03) << 0)];
		}
	...
	***************************************************************************/
	...
	{
		char red[] = {0x00, 0x24, 0x49, 0x6D, 0x92, 0xB6, 0xDB, 0xFF };
		char grn[] = {0x00, 0x24, 0x49, 0x6D, 0x92, 0xB6, 0xDB, 0xFF };
		char blu[] = {0x00, 0x55, 0xAA, 0xFF };
	...
	***************************************************************************/
	public static void segar_common_screenrefresh(mame_bitmap bitmap, int sprite_transparency, int copy_transparency)
	{
		int offs;
		int charcode;
