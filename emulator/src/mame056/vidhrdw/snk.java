...
public class snk
{
	...
	*******************************************************************************/
	...
	public static int MAX_VRAM_SIZE = (64*64*2); /* 0x2000 */
	...
	public static VhConvertColorPromPtr snk_3bpp_shadow_vh_convert_color_prom = new VhConvertColorPromPtr() {
            public void handler(char[] palette, char[] colortable, UBytePtr color_prom) {
                int i;
		palette_RRRR_GGGG_BBBB_convert_prom.handler(palette, colortable, color_prom);
	...
	public static VhConvertColorPromPtr snk_4bpp_shadow_vh_convert_color_prom = new VhConvertColorPromPtr() {
            public void handler(char[] palette, char[] colortable, UBytePtr color_prom) {
		int i;
		palette_RRRR_GGGG_BBBB_convert_prom.handler(palette, colortable, color_prom);
	...
	static void tnk3_draw_background( mame_bitmap bitmap, int scrollx, int scrolly )
	{
		...
		for( offs=0; offs<64*64*2; offs+=2 )
		{
			...
			if( tile_number!=dirtybuffer[offs] || attributes != dirtybuffer[offs+1] )
			{
				...
				dirtybuffer[offs+1] = (char) attributes;
	...
	public static void tnk3_draw_sprites( mame_bitmap bitmap, int xscroll, int yscroll )
	{
		...
		UBytePtr finish = new UBytePtr(source, 50*4);
		rectangle clip = Machine.visible_area;
	...
	public static void sgladiat_draw_background( mame_bitmap bitmap, int scrollx, int scrolly )
	{
		rectangle clip = Machine.visible_area;
		GfxElement gfx = Machine.gfx[1];
		int offs;
	...
	{
		UBytePtr pMem = memory_region(REGION_CPU1);
	...
	public static void ikari_draw_background( mame_bitmap bitmap, int xscroll, int yscroll ){
		...
		for( offs=0; offs<32*32*2; offs+=2 ){
			...
			int attributes = source.read(offs+1);
	...
	public static void ikari_draw_text( mame_bitmap bitmap ){
		rectangle clip = Machine.visible_area;
		GfxElement gfx = Machine.gfx[0];
	...
	//	shadows_visible = !shadows_visible;
		...
		{
			int bg_attributes = ram.read(0xc880);
		...
		{
			int sprite_attributes = ram.read(0xc900);
		...
		{
			int text_attributes = ram.read(0xc8c0);
	...
	public static VhUpdatePtr gwar_vh_screenrefresh = new VhUpdatePtr() { public void handler(mame_bitmap bitmap,int full_refresh) {
		UBytePtr ram = memory_region(REGION_CPU1);
		int bg_attributes, sp_attributes;
	...
	
		{
			...
			if( gwar_sprite_placement != 0 ) /* gwar */
			{
				...
				int spp_attributes = ram.read(0xca80);
		...
		{
			if( gwar_sprite_placement==2) { /* Gwar alternate */
				int text_attributes = ram.read(0xf8c0);
