...
import static mame056.commonH.REGION_SOUND1;
...
public class cclimber
{
	...
	public static int SND_CLOCK = 3072000;	/* 3.072 MHz */
	
	
	static UBytePtr samplebuf;	/* buffer to decode samples at run time */
	static int channel;
	...
	public static ShStartPtr cclimber_sh_start = new ShStartPtr() {
		...
		{
			samplebuf = new UBytePtr(2*memory_region_length(REGION_SOUND1));
			if (samplebuf == null)
				return 1;
		}
	...
	static void cclimber_play_sample(int start,int freq,int volume)
	{
		...
		while (start + len < memory_region_length(REGION_SOUND1) && rom.read(start+len) != 0x70)
		{
			...
			samplebuf.write(2*len, SAMPLE_CONV4(sample) * volume / 31);
	
			sample = rom.read(start + len) & 0x0f;
			samplebuf.write(2*len + 1, SAMPLE_CONV4(sample) * volume / 31);
