        ...
        public void handler(int chip, ShortPtr buffer, int length) {
            ...
            while (length > 0) {
                ...
                if (channel == -1) {
                    ...
                    buffer.write(0, (short) ((sum > 0x7fff) ? 0x7fff : sum));
                    buffer.offset += 2;
                    length--;
                } else {
                    int toggle = 0;
...
/*TODO*///
    ...
    static void rand_init(char[] rng, int size, int left, int right, int add) {
        ...
        for (int i = 0; i < mask; i++) {
            if (size == 17) {
                rng[ri] = (char) ((x >>> 6) & 0xFF);/* use bits 6..13 */
            } else {
                rng[ri] = (char) (x & 0xFF);/* use bits 0..7 */
            }
            //LOG_RAND(("%05x: %02x\n", x, *rng));
            ri++;
            /* calculate next bit */
    ...
    public int start(MachineSound msound) {
        ...
        for (chip = 0; chip < intf.num; chip++) {
            ...
            if (_pokey[chip].channel == -1) {
                printf("failed to initialize sound_old channel");
                return 1;
            }
        }
...
/*TODO*///
            case RANDOM_C:
                ...
                if ((_pokey[chip].SKCTL & SK_RESET) != 0) {
                    long adjust = (long) (timer_timeelapsed(_pokey[chip].rtimer) * intf.baseclock + 0.5);
