...
import static common.ptr.ShortPtr;

import static WIP.mame056.sound._5220intfH.IrqPtr;
import static WIP.mame056.sound.tms5220r.*;
...
public class tms5220 {
    ...
    static int[]/*UINT8*/ u8_fifo = new int[FIFO_SIZE];
    ...
    static int /*UINT8*/ u8_interp_count;
    /* number of interp periods (0-7) */
    static int /*UINT8*/ u8_sample_count;
        ...
        for (;;) {
            ...
            if (u8_speak_delay_frames != 0) {
                if (size <= u8_speak_delay_frames) {
                    u8_speak_delay_frames = (u8_speak_delay_frames - size) & 0xFF;
                    size = 0;
                } else {
...
//empty:
    ...
    static void process_command() {
        ...
        if (u8_fifo_count > 0) {
            u8_cmd = u8_fifo[u8_fifo_head] & 0x70;
            u8_fifo_count = (u8_fifo_count - 1) & 0xFF;
            u8_fifo_head = ((u8_fifo_head + 1) % FIFO_SIZE) & 0xFF;
    ...
    static int parse_frame(int removeit) {
        ...
        old_count = u8_fifo_count & 0xFF;
        old_head = u8_fifo_head & 0xFF;
        old_taken = u8_bits_taken & 0xFF;
